...
class QReCCBenchmark(BenchmarkRunner):
    ...
    def download(self) -> None:
        ...
        self._dataset = load_dataset("svakulenk0/qrecc", cache_dir=cache_dir)
    ...
    def load_data(self) -> None:
        ...
        # Collect all gold passages for building distractor pool
        all_gold_passages = {}
        for _conv_id, turns in conv_turns.items():
            for turn in turns:
                for passage in turn.get("Gold_passages", []) or []:
                    pid = f"passage_{len(all_gold_passages)}"
                    all_gold_passages[pid] = passage
        self.all_passages = all_gold_passages

        # Build conversation list
        conv_ids = sorted(conv_turns.keys())[: self.max_conversations]
                ...
                conv["turns"].append(
                    {
                        ...
                        "gold_passages": turn.get("Gold_passages", []) or [],
